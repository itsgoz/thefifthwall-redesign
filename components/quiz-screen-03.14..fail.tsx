...
const { quizState, nextQuestion, submitAnswer } = useQuiz();
...
// Guard — wait until questions are loaded
...
// Guard — wait until current question exists
if (!contextQuestion) {
  return null;
}
...
const q: QuizQuestion = {
  ...
  mediaLabel: contextQuestion.media?.url ?? "Media: image, gif or video",
  options: [
  ...
  correctKey: (["A","B","C","D"][
    contextQuestion.answerOptions.indexOf(contextQuestion.correctAnswer)
  ] ?? "A") as AnswerKey,
...
const isCorrect = selected === q.correctKey;
 ...
 const handleSubmit = () => {
  if (!selected) return;
  const selectedText = q.options.find(o => o.key === selected)?.text ?? ""
  ...
  const handleNext = () => {
    ...
    nextQuestion()  // ← tells the context to advance, and sets isComplete on the 10th
  };
